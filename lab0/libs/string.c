 ...
 * */
// 定义函数memset，它接受三个参数：一个指向要设置的内存区域的指针s，一个字符c，以及一个字节数n，用于将某块内存的前n个字节初始化为指定的值。
void *
memset(void *s, char c, size_t n) {
// 如果定义了__HAVE_ARCH_MEMSET
#ifdef __HAVE_ARCH_MEMSET
 // 则调用相应体系结构下的__memset函数实现内存设置
    return __memset(s, c, n);
// 如果没有定义__HAVE_ARCH_MEMSET
#else
    // 创建一个新的字符指针p，并将其初始化为s
    char *p = s;
    // 创建一个while循环，只要n大于0，就将c复制到p所指向的内存位置，并使p增加1，n减少1
    while (n -- > 0) {
        *p ++ = c;
    }
    // 在完成内存设置后，返回原始的内存指针s
