...
#define PADDR(kva)                                                 \
    ({                                                             \
        ...
        if (__m_kva < KERNBASE) {             /*// = 0x80200000(物理内存里内核的起始位置, KERN_BEGIN_PADDR) + 0xFFFFFFFF40000000(偏移量, PHYSICAL_MEMORY_OFFSET)*/                     \
...
static inline uintptr_t page2pa(struct Page *page) {
    ...
    //通过page2ppn获得页号（page number），然后通过左移PGSHIFT位（这通常是将页号转化为物理地址的操作）得到物理地址。
}
...
//将一个页的物理地址转换为内核虚拟地址
//page2pa(page)这部分会将Page结构体指针转换为该页的物理地址，然后KADDR宏将这个物理地址转换为内核空间的虚拟地址

static inline struct Page *kva2page(void *kva) { return pa2page(PADDR(kva)); }
//从页表项得到对应的页，这里用到了 PTE_ADDR(pte)宏，对页表项做操作
