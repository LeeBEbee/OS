...
#define swap_offset(entry) ({                                       \
               size_t __offset = (entry >> 8);    /*通过右移8位计算entry的偏移量__offset*/   \
               if (!(__offset > 0 && __offset < max_swap_offset)) {     /*检查计算出来的__offset是否在有效范围内*/ \
                    panic("invalid swap_entry_t = %08x.\n", entry);    \
               }                                                    \
               __offset;                                            \
          })
//在页表项（Page Table Entry，PTE）中，可能需要用到低8位来存储其他的元信息，如页权限等。因此，在存储交换空间的偏移量时，会选择将其存储在高位，这样就需要通过右移8位来获取这个偏移量。
