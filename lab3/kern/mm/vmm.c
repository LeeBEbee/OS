...
do_pgfault(struct mm_struct *mm, uint_t error_code, uintptr_t addr) {
    ...
    pte_t *ptep=NULL;//macro宏，DEFINEs定义
    /*
    ...
    *   pgdir_alloc_page : call alloc_page & page_insert functions to allocate（分配） a page size memory & setup
    ...
    *
    */
/* get_pte : 获取一个pte并返回这个pte的内核虚拟地址
* 如果页表中不存在这个pte，为页表分配一个页面（注意第三个参数’1’）
* pgdir_alloc_page : 调用alloc_page和page_insert函数来分配一个页面大小的内存，并用线性地址la和页目录表pgdir建立一个地址映射pa<—>la
* 定义：
* VM_WRITE : 如果 vma->vm_flags & VM_WRITE == 1/0，那么vma是可写的/不可写的
